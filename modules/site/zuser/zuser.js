...
function route(req, res, module, app, next) {
  ...
  // Menu
  // res.menu.admin.addMenuItem({name:'Profile',path:'zuser',url:'/zuser',description:'Your Profile ...',security:[]});

  res.menu.admin.addMenuItem({name:'Users', path: 'admin/zusers', weight: 10, url: '/zuser/list', description: 'Manage users ...', security: [] });
  res.menu.admin.addMenuItem({name:'Logout', path:'admin/logout', weight: 100, url: '/zuser/logout', description: 'Logout', security: [] });
...
function init(module, app, next) {
  ...
  calipso.lib.step(
    ...
    function defineRoutes() {
      module.router.addRoute(/.*/, loginForm, { end: false, template: 'login', block: 'zuser.login' }, this.parallel());
      module.router.addRoute('GET /zuser/login', loginPage, { end: false, template: 'login', block: 'content' }, this.parallel());
      module.router.addRoute('POST /zuser/login',loginUser,null,this.parallel());
      ...
      module.router.addRoute('GET /zuser/profile/:username/edit',updateUserForm,{block:'content'},this.parallel());

      module.router.addRoute('GET /zuser/profile/:username/lock',lockUser,{admin:true},this.parallel());
      module.router.addRoute('GET /zuser/profile/:username/unlock',unlockUser,{admin:true},this.parallel());
      module.router.addRoute('GET /zuser/profile/:username/delete',deleteUser,{admin:true},this.parallel());

	  /* zhongyi01的路由设置 */
      module.router.addRoute('GET /zuser/signup',signupUserForm,{template:'signup',block:'content'},this.parallel());
      module.router.addRoute('POST /zuser/signup',signupUser,null,this.parallel());
...
/**
 * Helper function to get zuser details respecting their privacy selections
 * Returns object of:
 * {
...
function loginForm(req, res, template, block, next) {
  ...
  var userForm = {
    id:'login-form',cls:'login',title:'Log In',type:'form',method:'POST',action:'/zuser/login',
    fields:[
    ...
    buttons:[
      ...
      {name:'register', type:'button', link:'/zuser/register', value:'Register'}
    ]
  };
...
function registerUserForm(req, res, template, block, next) {
  ...
  // TODO : Use secitons!
  var userForm = {
    id:'FORM',title:req.t('Register'),type:'form',method:'POST',action:'/zuser/register',
    sections:[{
...
function updateUserForm(req, res, template, block, next) {
  ...
  if(isAdmin) {
      res.menu.adminToolbar.addMenuItem({name:'Return',path:'return',url:'/zuser/profile/'+username,description:'Show user ...',security:[]});
  }

  var userForm = {
    id:'FORM',title:'Update Profile',type:'form',method:'POST',tabs:true,action:'/zuser/profile/' + username,
    sections:[{
      id:'form-section-core',
      label:'Profile',
      fields:[
...
function lockUser(req, res, template, block, next) {
  ...
  User.findOne({username:username}, function(err, u) {
      ...
      u.save(function(err) {
          ...
          res.redirect('/zuser/profile/' + username);
      });
        ...
        u.save(function(err) {
          if(err) {
            ...
            if(res.statusCode != 302) {
              ...
              res.redirect('/zuser/profile/' + username + '/edit');
            }
            ...
            res.redirect('/zuser/profile/' + u.username);
...
function userProfile(req, res, template, block, next) {
  ...
  User.findOne({username:username}, function(err, u) {
    ...
    if(req.session.user && req.session.user.isAdmin) {
        res.menu.adminToolbar.addMenuItem({name:'List',weight:2,path:'list',url:'/zuser/list',description:'List users ...',security:[]});
        res.menu.adminToolbar.addMenuItem({name:'Edit',weight:1,path:'edit',url:'/zuser/profile/' + username + '/edit',description:'Edit user details ...',security:[]});
        res.menu.adminToolbar.addMenuItem({name:'Delete',weight:3,path:'delete',url:'/zuser/profile/' + username + '/delete',description:'Delete account ...',security:[]});

        if(!u.locked) res.menu.adminToolbar.addMenuItem({name:'Lock',weight:5,path:'lock',url:'/zuser/profile/' + username + '/lock',description:'Lock account ...',security:[]});
        if(u.locked) res.menu.adminToolbar.addMenuItem({name:'Unlock',weight:4,path:'unlock',url:'/zuser/profile/' + username + '/unlock',description:'Unlock account ...',security:[]});
    }
...
function userLink(req,user) {
  return calipso.link.render({id:user._id,title:req.t('View {user}',{user:user.username}),label:user.username,url:'/zuser/profile/' + user.username});
}
...
function listUsers(req,res,template,block,next) {
      ...
      res.menu.adminToolbar.addMenuItem({name:'Register New User',path:'new',url:'/zuser/register',description:'Register new user ...',security:[]});
 ...
 * 接收注册数据并处理
 */
function signupUser(req, res, template, block, next) {
    ...
    if(form) {
      ...
      u.save(function(err) {
        ...
        if(err) {
          ...
          calipso.e.post_emit('USER_CREATE',u);
          if(!res.noRedirect) {
            req.flash('info',req.t('账户已经创建，现在您可以使用这个账户登录.'));
            res.redirect('/zuser/profile/' + u.username);
          }
        }
